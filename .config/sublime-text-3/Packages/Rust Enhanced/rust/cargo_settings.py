...
class CargoSettings(object):
    ...
    def get_command(self, cmd_name, cmd_info,
        ...
        # Command to run.
        if cargo_target == None or cargo_target in targets_with_sysroot:
            result.append(cmd_info['command'])
        else:
            # Need to make xbuild the else-branch so all custom targets land here
            result.append("x" + cmd_info['command'])
